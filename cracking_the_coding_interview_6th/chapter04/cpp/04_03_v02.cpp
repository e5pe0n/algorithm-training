...
vector<vector<Node *>> make_depth_list(Tree *tree) {
  ...
  res.push_back(vector<Node *>{tree->root});
  q.push({tree->root, 0});
  while (!q.empty()) {
    auto [par, depth] = q.front();
    q.pop();
    if (res.size() <= depth + 1) res.push_back(vector<Node *>());
    if (par->left != nullptr) {
      res[depth + 1].push_back(par->left);
      q.push({par->left, depth + 1});
    }
    if (par->right != nullptr) {
      res[depth + 1].push_back(par->right);
      q.push({par->right, depth + 1});
    }
  }
  return res;
}
