...
ll clear_bits_msb_though_0(ll n, ll i) {
  ll mask = (1 << i) - 1;
  return n & mask;
}
...
int main() {
  ...
  // bitset<4> after2(clear_bit(n, 1));
  // puts(before.to_string().c_str()); // 0111
  // puts(after2.to_string().c_str()); // 0101
  // puts("");

  // bitset<4> after3(clear_bits_msb_though_0(n, 2));
  // puts(before.to_string().c_str()); // 0111
  // puts(after3.to_string().c_str()); // 0011
  // puts("");

  bitset<4> after4(clear_bits_i_through_0(n, 1));
  puts(before.to_string().c_str()); // 0111
  puts(after4.to_string().c_str()); // 0100
  puts("");

  // bitset<4> after5(update_bit(n, 1, 0));
  // puts(before.to_string().c_str()); // 0111
  // puts(after5.to_string().c_str()); // 0101
  // puts("");
}
