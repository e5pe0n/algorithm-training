...
ll clear_bits_i_though_0(ll n, ll i) {
  ll mask = -1LL << (i + 1);
  return n & mask;
}

ll clear_bits_by_range(ll n, ll i, ll j) {
  // clear bits in [i, j) where i and j is base-0 index respectively
  return clear_bits_msb_though_i(n, i) | clear_bits_i_though_0(n, j - 1);
}
...
ll update_bits(ll n, ll m, ll start, ll end) {
  // update n in binary in [start, end)
  // where start and end is base-0 index respectivly
  return clear_bits_by_range(n, start, end) | (m << start);
}
