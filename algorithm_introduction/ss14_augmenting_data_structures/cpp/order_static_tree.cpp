...
struct Tree {
  ...
  void _os_transplant(Node *u, Node *v) {
    if (u->p == &nil)
      root = v;
    else if (u == u->p->left)
      u->p->left = v;
  ...
  void os_delete(Node *z) {
    ...
    if (z->left == &nil) {
      x = z->right;
      _os_transplant(z, z->right);
    } else if (z->right == &nil) {
      x = z->left;
      _os_transplant(z, z->left);
    } else {
