...
class Tree:
    ...
    def _os_transplant(self, u: Node, v: Node):
        if u is self.root:
            self.root = v
        elif u.p.left is u:
            u.p.left = v
    ...
    def _os_insert_fixup(self, z: Node):
    ...
    def os_insert(self, z: Node):
        ...
        z.right = self.nil
        z.color = Color.RED
        self._os_insert_fixup(z)

    def _os_delete_fixup(self, x: Node):
    ...
    def os_delete(self, z: Node):
        y = z
        y_original_color = y.color
        if z.left is self.nil:
            x = z.right
            self._os_transplant(z, z.right)
        elif z.right is self.nil:
            x = z.left
            self._os_transplant(z, z.left)
        else:
            ...
            if y.p == z:
                ...
                self._os_transplant(y, y.right)
                y.right = z.right
                y.right.p = y
            self._os_transplant(z, y)
            y.left = z.left
            y.left.p = y
            y.color = z.color
    ...
    def os_select(self, x: Node, i: int) -> Node:
        r = x.left.size + 1
        if i == r:
            return x
        elif i < r:
            ...
            return self.os_select(x.right, r - i)

    def os_rank(self, x: Node) -> int:
        r = x.left.size + 1
